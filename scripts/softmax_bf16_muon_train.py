"""
SoftmaxBF16Muon training: SVD-based softmax-weighted gradient updates.
...
from nanochat.optim_softmax_bf16 import SoftmaxBF16MuonAdamW, DistSoftmaxBF16MuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="SoftmaxBF16Muon training")
...
# Newton-Schulz not used by SoftmaxBF16Muon, but needed for param grouping compatibility
_ns_steps = 5
...
output_dirname = args.model_tag if args.model_tag else f"smxbf16_eta{args.eta}_d{args.depth}"
...
def setup_softmax_optimizer(model, eta):
    ...
    Factory = DistSoftmaxBF16MuonAdamW if is_ddp else SoftmaxBF16MuonAdamW
    optimizer = Factory(param_groups, eta=eta)
    for group in optimizer.param_groups:
        group["initial_lr"] = group["lr"]
    return optimizer

optimizer = setup_softmax_optimizer(orig_model, eta=args.eta)
print0(f"SoftmaxBF16Muon optimizer created with eta={args.eta}")
...
get_report().log(section=f"SoftmaxBF16Muon (eta={args.eta}) training", data=[
    user_config,
