"""
TaylorMuon training: first-order Taylor approximation of SoftmaxMuon.

Replaces SVD with Newton-Schulz + cheap linear correction:
    Q = newton_schulz(M)
    M_scaled = M / (||M|| / sqrt(r))
    update = Q + eta * M_scaled
    update = update * (sqrt(r) / ||update||)

Zero SVD, fully torch.compile-able, one extra matrix add beyond Muon.

Run as:
    torchrun --standalone --nproc_per_node=2 -m scripts.softmax_taylor_muon_train -- --depth 20 --eta 0.5 ...
"""
...
from nanochat.optim_taylor import TaylorMuonAdamW, DistTaylorMuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="TaylorMuon training")
...
output_dirname = args.model_tag if args.model_tag else f"taylor_eta{args.eta}_d{args.depth}"
...
def setup_taylor_optimizer(model, eta):
...
optimizer = setup_taylor_optimizer(orig_model, eta=args.eta)
print0(f"TaylorMuon optimizer created with eta={args.eta}")
...
get_report().log(section=f"TaylorMuon (eta={args.eta}) training", data=[
    user_config,
