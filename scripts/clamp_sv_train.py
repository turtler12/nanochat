"""
Clamp-or-random singular value mapping experiment for Muon optimizer.

Two modes, selected via --clamp-below:

  Pure random (default, --clamp-below not set):
    Every active normalized SV is replaced with Uniform[rand-lo, rand-hi].
    Identical to rand_sv_train.py.

  Clamp-or-random (--clamp-below):
    If normalized SV < rand-lo: clamp to rand-lo.
    If normalized SV >= rand-lo: replace with Uniform[rand-lo, rand-hi].

Usage:
  # pure random (same as rand_sv_train):
  torchrun --standalone --nproc_per_node=4 -m scripts.clamp_sv_train -- --depth 20

  # clamp small SVs, randomize the rest:
  torchrun --standalone --nproc_per_node=4 -m scripts.clamp_sv_train -- --depth 20 --clamp-below
...
def make_svd_map_random(lo: float = 0.1, hi: float = 1.0):
    """Every active normalized SV is replaced with Uniform[lo, hi]."""
    lo_val = float(lo)
    hi_val = float(hi)
    ...
    def _map(s: torch.Tensor) -> torch.Tensor:
        ...
        return torch.where(active, rand_vals, torch.zeros_like(rand_vals))

    return _map


def make_svd_map_clamp_or_random(threshold: float = 0.1, hi: float = 1.0):
    """
    Clamp-or-random mapping:
      - Normalize singular values by the max (so the largest = 1).
      - If normalized SV < threshold: set it to threshold (clamp up).
      - If normalized SV >= threshold: replace with Uniform[threshold, hi].

    Dead directions (normalized SV == 0) remain zero.
    """
    ...
    def _map(s: torch.Tensor) -> torch.Tensor:
        ...
        s_norm = s / s_max
        active = (s_norm > 0)
        # For active SVs below threshold, clamp to threshold
        clamped = s_norm.clamp_min(thresh)
        # For active SVs at or above threshold, draw random in [threshold, hi]
        rand_vals = torch.empty_like(s).uniform_(thresh, hi_val)
        s_new = torch.where(s_norm >= thresh, rand_vals, clamped)
        # Zero out dead directions
        s_new = torch.where(active, s_new, torch.zeros_like(s_new))
        return s_new
...
def muon_step_svd_clamp(
    ...
    """
    Muon step with clamp-or-random SV mapping instead of Polar Express.
    Same structure: momentum -> SVD clamp/random mapping -> variance reduction -> cautious update.
    """
    ...
    # Clamp-or-random SV mapping (replaces Polar Express)
...
class MuonAdamW_SVDClamp(MuonAdamW):
...
class DistMuonAdamW_SVDClamp(DistMuonAdamW):
...
def make_setup_optimizer_svd_clamp(svd_map_fn):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        Factory = DistMuonAdamW_SVDClamp if ddp else MuonAdamW_SVDClamp
...
parser = argparse.ArgumentParser(description="Clamp-or-random SV mapping experiment for Muon optimizer")
# Experiment-specific args
parser.add_argument("--rand-lo", type=float, default=0.1, help="Lower bound for random SV draw / clamp threshold (default 0.1)")
parser.add_argument("--rand-hi", type=float, default=1.0, help="Upper bound for random SV draw (default 1.0)")
parser.add_argument("--clamp-below", action="store_true", help="If set, SVs below rand-lo are clamped to rand-lo instead of getting a random draw")
parser.add_argument("--results-dir", type=str, default="clamp_sv_results", help="Directory to save results")
# Logging
...
if args.clamp_below:
    svd_map_fn = make_svd_map_clamp_or_random(threshold=args.rand_lo, hi=args.rand_hi)
    run_tag = f"clamp_sv_lo{args.rand_lo:.2f}_hi{args.rand_hi:.2f}"
    print0(f"=== Clamp-or-random SV mapping: clamp below {args.rand_lo:.2f}, random in [{args.rand_lo:.2f}, {args.rand_hi:.2f}] above ===")
else:
    svd_map_fn = make_svd_map_random(lo=args.rand_lo, hi=args.rand_hi)
    run_tag = f"rand_sv_lo{args.rand_lo:.2f}_hi{args.rand_hi:.2f}"
    print0(f"=== Random SV mapping: Uniform[{args.rand_lo:.2f}, {args.rand_hi:.2f}] ===")

# Monkey-patch GPT.setup_optimizer
GPT.setup_optimizer = make_setup_optimizer_svd_clamp(svd_map_fn)
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="nanochat-clamp-sv", name=f"{args.run}_{run_tag}", config=user_config)
