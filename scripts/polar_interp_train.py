"""
Train model with Polar Interpolation v2 optimizer.

v2 approach: Let Muon do its full job (Polar Express + NorMuon), then blend
in gradient magnitude info as a post-NorMuon correction:

  g_muon = NorMuon(PolarExpress(momentum_grad))   # standard Muon output
  g_raw  = normalize(momentum_grad, target_norm=||g_muon||)
  g_final = (1-β)·g_muon + β·g_raw, renormalized to ||g_muon||

At β=0: identical to standard Muon
At β>0: blends in raw gradient direction info while preserving Muon's update scale

Environment variables:
  BETA_INTERP=0.3     Interpolation strength (0.0 = Muon, higher = more raw gradient)
  NS_STEPS=5          Newton-Schulz iterations (5 = same as baseline)

Usage:
  BETA_INTERP=0.3 NS_STEPS=5 torchrun --standalone --nproc_per_node=4 \
      -m scripts.polar_interp_train -- --depth 20 --fp8 ...
"""
...
from scripts.muon_polar_interp import (
    MuonAdamW as MuonAdamW_PolarInterp,
    DistMuonAdamW as DistMuonAdamW_PolarInterp,
)
print_banner()
...
BETA_INTERP = float(os.environ.get("BETA_INTERP", "0.3"))
NS_STEPS = int(os.environ.get("NS_STEPS", "5"))

print0(f"=== Polar Interpolation v2 Experiment ===")
print0(f"  BETA_INTERP={BETA_INTERP}, NS_STEPS={NS_STEPS}")

# =============================================================================
# Monkey-patch GPT.setup_optimizer to use Polar Interpolation optimizer
# =============================================================================

def make_setup_optimizer_polar_interp(beta_interp: float, ns_steps: int):
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        Factory = DistMuonAdamW_PolarInterp if ddp else MuonAdamW_PolarInterp
...
GPT.setup_optimizer = make_setup_optimizer_polar_interp(BETA_INTERP, NS_STEPS)

# =============================================================================
# CLI arguments (identical to base_train.py)
# =============================================================================

parser = argparse.ArgumentParser(description="Pretrain with Polar Interpolation optimizer")
...
# =============================================================================
# The rest is identical to base_train.py
# =============================================================================

device_type = autodetect_device_type() if args.device_type == "" else args.device_type
ddp, ddp_rank, ddp_local_rank, ddp_world_size, device = compute_init(device_type)
# Fix TF32 API conflict: compute_init uses the legacy torch.backends.fp32_precision = "tf32",
# but torch.compile's inductor checks the new API (torch.backends.cuda.matmul.allow_tf32).
# Set both to avoid the "mixed legacy and new APIs" RuntimeError.
if device_type == "cuda":
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(
    project="nanochat", name=args.run,
    config={**user_config, "beta_interp": BETA_INTERP, "ns_steps": NS_STEPS},
)
...
get_report().log(section="Polar Interpolation training", data=[
    user_config,
    {
        "beta_interp": BETA_INTERP, "ns_steps": NS_STEPS,
        "power_iters": POWER_ITERS, "power_batch": POWER_BATCH,
