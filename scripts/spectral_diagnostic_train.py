"""
Spectral diagnostic training: track nuclear norm, SV entropy, Frobenius norm,
and spectral norm of a representative MLP up-projection layer every 50 steps.

Supports both Muon (default) and pure-Adam (--optimizer adam) modes.

Run as:
    torchrun --standalone --nproc_per_node=2 -m scripts.spectral_diagnostic_train -- --depth 20 --optimizer muon ...
    torchrun --standalone --nproc_per_node=2 -m scripts.spectral_diagnostic_train -- --depth 20 --optimizer adam ...
"""
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Spectral diagnostic training")
...
# Optimization
parser.add_argument("--optimizer", type=str, default="muon", choices=["muon", "adam"], help="optimizer: muon (default) or adam (AdamW for all params)")
...
parser.add_argument("--adam-matrix-lr", type=float, default=3e-4, help="learning rate for matrix params when using Adam optimizer")
...
# Spectral diagnostic
parser.add_argument("--diag-every", type=int, default=50, help="log spectral diagnostics every N steps")
parser.add_argument("--diag-layer", type=int, default=5, help="which layer's MLP up-projection to track")
# Output
parser.add_argument("--model-tag", type=str, default=None)
args = parser.parse_args()
user_config = vars(args).copy()

# Newton-Schulz iteration count from mode
...
# -----------------------------------------------------------------------------
# Spectral diagnostic function

def compute_spectral_diagnostics(model, layer_idx):
    """Compute spectral diagnostics for the MLP up-projection of the given layer."""
    target_name = f"transformer.h.{layer_idx}.mlp.c_fc.weight"
...
if args.optimizer == "muon":
    print0("Optimizer: Muon (matrix params) + AdamW (embeddings/scalars)")
    ...
    # Pure AdamW for ALL parameters (no Muon)
    print0(f"Optimizer: Pure AdamW for all parameters (matrix_lr={args.adam_matrix_lr})")
    adam_matrix_lr = args.adam_matrix_lr * batch_lr_scale
    model_dim = model_config.n_embd
    dmodel_lr_scale = (model_dim / 768) ** -0.5

    param_groups = [
        dict(params=list(orig_model.lm_head.parameters()), lr=args.unembedding_lr * batch_lr_scale * dmodel_lr_scale,
             betas=(args.adam_beta1, args.adam_beta2), eps=1e-10, weight_decay=0.0),
        dict(params=list(orig_model.transformer.wte.parameters()), lr=args.embedding_lr * batch_lr_scale * dmodel_lr_scale,
             betas=(args.adam_beta1, args.adam_beta2), eps=1e-10, weight_decay=0.0),
        dict(params=list(orig_model.value_embeds.parameters()), lr=args.embedding_lr * batch_lr_scale * dmodel_lr_scale,
             betas=(args.adam_beta1, args.adam_beta2), eps=1e-10, weight_decay=0.0),
        dict(params=[orig_model.resid_lambdas], lr=args.scalar_lr * 0.01 * batch_lr_scale,
             betas=(args.adam_beta1, args.adam_beta2), eps=1e-10, weight_decay=0.0),
        dict(params=[orig_model.x0_lambdas], lr=args.scalar_lr * batch_lr_scale,
             betas=(0.96, 0.95), eps=1e-10, weight_decay=0.0),
        # All transformer.h matrix params go through AdamW too
        dict(params=list(orig_model.transformer.h.parameters()), lr=adam_matrix_lr,
             betas=(args.adam_beta1, args.adam_beta2), eps=1e-8, weight_decay=weight_decay_scaled),
    ]
...
diag_log_path = os.path.join(checkpoint_dir, "spectral_diag.jsonl")
...
# Identify the tracked layer
diag_layer_name = f"transformer.h.{args.diag_layer}.mlp.c_fc.weight"
print0(f"Tracking spectral diagnostics for: {diag_layer_name} (every {args.diag_every} steps)")
...
while True:
    ...
    if master_process and args.diag_every > 0 and (last_step or step % args.diag_every == 0):
        diag = compute_spectral_diagnostics(orig_model, args.diag_layer)
        diag["step"] = step
        diag["total_training_time"] = total_training_time
        with open(diag_log_path, "a") as f:
            f.write(json.dumps(diag) + "\n")
        print0(f"Step {step:05d} | nuclear={diag['nuclear_norm']:.1f} entropy={diag['sv_entropy']:.3f} fro={diag['frobenius_norm']:.3f} spectral={diag['spectral_norm']:.4f}")
    ...
    for group in optimizer.param_groups:
        ...
        if args.optimizer == "muon" and group['kind'] == 'muon':
            group["momentum"] = get_muon_momentum(step)
            group["weight_decay"] = get_weight_decay(step)
        elif args.optimizer == "adam" and group.get("weight_decay", 0) > 0:
            # Decay weight decay for Adam matrix params too
            group["weight_decay"] = get_weight_decay(step)
    optimizer.step()
...
get_report().log(section=f"Spectral diagnostic ({args.optimizer})", data=[
    user_config,
