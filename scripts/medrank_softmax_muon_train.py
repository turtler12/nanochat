"""
MedRankSoftmaxMuon training: medium-rank SVD: SVD-based softmax-weighted gradient updates.

Replaces Polar Express orthogonalization with:
  U, S, Vh = SVD(G)
  p = softmax(eta * S)
  tau = min(m, n)  — matches Muon's orthogonal update nuclear norm
  update = U @ diag(tau * p) @ Vh

Run as:
    torchrun --standalone --nproc_per_node=2 -m scripts.softmax_muon_train -- --depth 20 --eta 1.0 ...
"""
...
from nanochat.optim_medrank_softmax import MedRankSoftmaxMuonAdamW, DistMedRankSoftmaxMuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="MedRankSoftmaxMuon training: medium-rank SVD")
...
# MedRankSoftmaxMuon-specific
parser.add_argument("--eta", type=float, default=1.0, help="softmax temperature for singular value weighting")
parser.add_argument("--rank-k", type=int, default=64, help="max rank for randomized SVD (capped at r//4)")
...
# Newton-Schulz not used by SoftmaxMuon, but needed for param grouping compatibility
_ns_steps = 5
...
output_dirname = args.model_tag if args.model_tag else f"medrank_eta{args.eta}_d{args.depth}"
...
# First MLP up-projection: transformer.h[0].mlp.c_fc
...
# -----------------------------------------------------------------------------
# Custom optimizer setup (replaces model.setup_optimizer)
def setup_medrank_optimizer(model, eta, rank_k):
    ...
    Factory = DistMedRankSoftmaxMuonAdamW if is_ddp else MedRankSoftmaxMuonAdamW
    optimizer = Factory(param_groups, eta=eta, rank_k=rank_k)
    for group in optimizer.param_groups:
        group["initial_lr"] = group["lr"]
    return optimizer

optimizer = setup_medrank_optimizer(orig_model, eta=args.eta, rank_k=args.rank_k)
print0(f"MedRankSoftmaxMuon optimizer created with eta={args.eta}, rank_k={args.rank_k}")
...
get_report().log(section=f"MedRankSoftmaxMuon (eta={args.eta}, k={args.rank_k}) training", data=[
    user_config,
