"""
Randomized-SVD SoftmaxMuon training: uses randomized rank-k SVD for softmax
reweighting of top singular directions, with standard Muon for the rest.

Run as:
    torchrun --standalone --nproc_per_node=2 -m scripts.randomized_softmax_muon_train -- --depth 20 --eta 0.5 --k 64 ...
"""
...
from nanochat.optim_randomized_softmax import RandomizedSoftmaxMuonAdamW, DistRandomizedSoftmaxMuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Randomized-SVD SoftmaxMuon training")
...
# Randomized SoftmaxMuon-specific
parser.add_argument("--eta", type=float, default=0.5, help="softmax temperature for singular value weighting")
parser.add_argument("--k", type=int, default=64, help="number of top singular triplets for randomized SVD")
parser.add_argument("--n-power-iter", type=int, default=2, help="number of power iterations for randomized SVD")
...
output_dirname = args.model_tag if args.model_tag else f"rsvd_k{args.k}_eta{args.eta}_d{args.depth}"
...
# -----------------------------------------------------------------------------
# Optimizer setup
def setup_randomized_softmax_optimizer(model, eta, k, n_power_iter):
    ...
    Factory = DistRandomizedSoftmaxMuonAdamW if is_ddp else RandomizedSoftmaxMuonAdamW
    optimizer = Factory(param_groups, eta=eta, k=k, n_power_iter=n_power_iter)
    for group in optimizer.param_groups:
        group["initial_lr"] = group["lr"]
    return optimizer

optimizer = setup_randomized_softmax_optimizer(orig_model, eta=args.eta, k=args.k, n_power_iter=args.n_power_iter)
print0(f"Randomized-SVD SoftmaxMuon optimizer: eta={args.eta}, k={args.k}, n_power_iter={args.n_power_iter}")
...
get_report().log(section=f"RandomizedSVD SoftmaxMuon (eta={args.eta}, k={args.k}) training", data=[
    user_config,
