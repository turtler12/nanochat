"""
SoftmaxMuon training: SVD-based softmax-weighted gradient updates.
...
from nanochat.optim_softmax import SoftmaxMuonAdamW, DistSoftmaxMuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="SoftmaxMuon training")
...
output_dirname = args.model_tag if args.model_tag else f"softmax_eta{args.eta}_d{args.depth}"
...
def setup_softmax_optimizer(model, eta):
    ...
    Factory = DistSoftmaxMuonAdamW if is_ddp else SoftmaxMuonAdamW
...
print0(f"SoftmaxMuon optimizer created with eta={args.eta}")
...
get_report().log(section=f"SoftmaxMuon (eta={args.eta}) training", data=[
    user_config,
