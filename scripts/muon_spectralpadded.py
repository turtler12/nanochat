...
def muon_step_fused(
    ...
    # Spectral padding blend (cheap hard-ish floor proxy):
    #   update = (1-γ) * g_raw + γ * σ̂_max(g_raw) * P
    # This maps singular values roughly as σ' ≈ (1-γ)σ + γσ_max, so σ' ≥ γσ_max.
    pad_ratio = pad_ratio_t.to(g.dtype)

    # Estimate σ_max(g_raw) with a few batched power iterations using GEMMs.
    # NOTE: much cheaper than full SVD and typically small relative to Polar Express cost.
    b = power_batch
    ...
    # Blend: if pad_ratio=0 this degenerates to g (raw); if pad_ratio=1 it becomes σ̂_max * P.
