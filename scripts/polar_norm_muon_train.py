"""
PolarNormMuon training: nuclear norm normalization via the polar factor.

Muon already computes Q ≈ UV^T. The nuclear norm is just ⟨Q, G⟩_F — one dot product.
Scale the update by 1/nuclear_norm for nearly free nuclear norm normalization.

Run as:
    torchrun --standalone --nproc_per_node=2 -m scripts.polar_norm_muon_train -- --depth 20 --eta 1.0 ...
"""
...
from nanochat.optim_polar_norm import PolarNormMuonAdamW, DistPolarNormMuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="PolarNormMuon training")
...
# PolarNormMuon-specific
parser.add_argument("--eta", type=float, default=1.0, help="Nuclear norm normalization strength (0=Muon, 1=full)")
...
output_dirname = args.model_tag if args.model_tag else f"polar_norm_eta{args.eta}_d{args.depth}"
...
# -----------------------------------------------------------------------------
# Optimizer
def setup_polar_norm_optimizer(model, eta):
    ...
    Factory = DistPolarNormMuonAdamW if is_ddp else PolarNormMuonAdamW
    optimizer = Factory(param_groups, eta=eta)
    for group in optimizer.param_groups:
        group["initial_lr"] = group["lr"]
    return optimizer

optimizer = setup_polar_norm_optimizer(orig_model, eta=args.eta)
print0(f"PolarNormMuon optimizer created with eta={args.eta}")
...
while True:
    ...
    if should_log_spectral:
        ...
        spectral_entry = {"step": step, "sv_entropy": sv_entropy, "nuclear_norm": nuclear_norm, "eta": args.eta}
...
get_report().log(section=f"PolarNormMuon (eta={args.eta}) training", data=[
    user_config,
