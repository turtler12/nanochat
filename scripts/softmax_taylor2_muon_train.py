"""
TaylorMuon2 training: second-order Taylor approximation of SoftmaxMuon.

Replaces SVD with Newton-Schulz + quadratic correction:
    Q = newton_schulz(M)
    M_scaled = M * (sqrt(r) / ||M||)
    S2V = M_scaled @ M_scaled^T @ Q
    quad = (S2V - 2*S_bar*M_scaled + (2*S_bar^2 - 1)*Q) / r
    update = Q + eta * M_scaled + 0.5 * eta^2 * quad
    update = update * (sqrt(r) / ||update||)

Zero SVD, fully torch.compile-able, two extra matmuls beyond TaylorMuon.

Run as:
    torchrun --standalone --nproc_per_node=2 -m scripts.softmax_taylor2_muon_train -- --depth 20 --eta 0.5 ...
"""
...
from nanochat.optim_taylor2 import Taylor2MuonAdamW, DistTaylor2MuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="TaylorMuon2 (quadratic) training")
...
# TaylorMuon-specific
parser.add_argument("--eta", type=float, default=0.5, help="Taylor correction strength")
...
# Fix TF32 API conflict: compute_init uses the new API (torch.backends.fp32_precision),
# but torch.compile's inductor checks the legacy API. Set both to be safe.
if device_type == "cuda":
...
output_dirname = args.model_tag if args.model_tag else f"taylor2_eta{args.eta}_d{args.depth}"
...
def setup_taylor2_optimizer(model, eta):
    ...
    Factory = DistTaylor2MuonAdamW if is_ddp else Taylor2MuonAdamW
    optimizer = Factory(param_groups, eta=eta)
    for group in optimizer.param_groups:
        group["initial_lr"] = group["lr"]
    return optimizer

optimizer = setup_taylor2_optimizer(orig_model, eta=args.eta)
print0(f"TaylorMuon2 optimizer created with eta={args.eta}")
...
get_report().log(section=f"TaylorMuon2 (eta={args.eta}) training", data=[
    user_config,
