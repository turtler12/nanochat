...
_2GPU_ROWS = [
    ...
    ("ns3+ftrl η=0.1 (2gpu)",
     "ns3+ftrl η=0.1 (2gpu)", None,
     _OPT_FTRL * _TOTAL_STEPS / 1e3,
     (_OPT_FTRL + _FWD_BWD_PER_STEP) * _TOTAL_STEPS / 1e6),
    ("ns3+ftrl linear →0 (2gpu)",
     "ns3+ftrl linear η=0.3→0 (2gpu)", None,
     _OPT_FTRL * _TOTAL_STEPS / 1e3,
     (_OPT_FTRL + _FWD_BWD_PER_STEP) * _TOTAL_STEPS / 1e6),
    ("ns3+ftrl exp →0 (2gpu)",
     "ns3+ftrl exp η=0.3→0 (2gpu)", None,
     _OPT_FTRL * _TOTAL_STEPS / 1e3,
     (_OPT_FTRL + _FWD_BWD_PER_STEP) * _TOTAL_STEPS / 1e6),
...
ax.set_title("FLOPs Comparison (d12, 2gpu runs)", fontsize=9, pad=4)

plt.suptitle("Muon vs NS3+FTRL variants (depth-12, 2×H100, 2205 steps) — 2gpu runs", fontsize=12)
plt.tight_layout()
plt.savefig("/Users/medha/Desktop/muon_local/nanochat/plots/cluster_muon_vs_ns3ftrl_2gpu.png", dpi=150, bbox_inches="tight")
plt.show()
print(f"Crossover: step {crossover_step}, time {crossover_time:.1f}m")
print("Saved to plots/cluster_muon_vs_ns3ftrl_2gpu.png")
