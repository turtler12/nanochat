"""
LowRankSoftmaxMuon training: per-direction softmax reweighting via randomized
low-rank SVD. Approximates the full-SVD softmax experiment cheaply by only
computing top-k singular directions (softmax zeros out the rest anyway).

Run as:
    torchrun --standalone --nproc_per_node=2 -m scripts.lowrank_softmax_muon_train -- --depth 20 --eta 0.1 --rank-k 4 ...
"""
...
from nanochat.optim_lowrank_softmax import LowRankSoftmaxMuonAdamW, DistLowRankSoftmaxMuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="LowRankSoftmaxMuon training")
...
# LowRankSoftmaxMuon-specific
parser.add_argument("--eta", type=float, default=0.1, help="Softmax temperature (0=uniform/Muon, higher=more concentrated)")
parser.add_argument("--rank-k", type=int, default=4, help="Number of top singular directions for low-rank SVD")
...
output_dirname = args.model_tag if args.model_tag else f"lrsmx_k{args.rank_k}_eta{args.eta}_d{args.depth}"
...
# Spectral logging layer (before compile)
...
# -----------------------------------------------------------------------------
# Optimizer
def setup_optimizer(model, eta, rank_k):
    ...
    for shape in sorted({p.shape for p in matrix_params}):
        ...
        param_groups.append(dict(
            ...
            momentum=0.95, ns_steps=5, beta2=0.95, weight_decay=weight_decay_scaled,
        ))

    Factory = DistLowRankSoftmaxMuonAdamW if is_ddp else LowRankSoftmaxMuonAdamW
    optimizer = Factory(param_groups, eta=eta, rank_k=rank_k)
    for group in optimizer.param_groups:
        group["initial_lr"] = group["lr"]
    return optimizer

optimizer = setup_optimizer(orig_model, eta=args.eta, rank_k=args.rank_k)
print0(f"LowRankSoftmaxMuon optimizer created with eta={args.eta}, rank_k={args.rank_k}")
...
while True:
    ...
    if should_log_spectral:
        ...
        spectral_entry = {"step": step, "sv_entropy": sv_entropy, "nuclear_norm": nuclear_norm, "eta": args.eta, "rank_k": args.rank_k}
...
get_report().log(section=f"LowRankSoftmaxMuon (eta={args.eta}, k={args.rank_k}) training", data=[
    user_config,
