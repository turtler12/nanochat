"""
Cached-SVD SoftmaxMuon training: amortizes SVD cost by recomputing every svd_freq steps.

Run as:
    torchrun --standalone --nproc_per_node=2 -m scripts.cached_softmax_muon_train -- --depth 20 --eta 0.5 --svd-freq 10 ...
"""
...
from nanochat.optim_cached_softmax import CachedSoftmaxMuonAdamW, DistCachedSoftmaxMuonAdamW
...
# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Cached-SVD SoftmaxMuon training")
...
# Cached SoftmaxMuon-specific
parser.add_argument("--eta", type=float, default=0.5, help="softmax temperature for singular value weighting")
parser.add_argument("--svd-freq", type=int, default=10, help="recompute SVD every N steps (1 = no caching)")
...
output_dirname = args.model_tag if args.model_tag else f"cached_svd_freq{args.svd_freq}_eta{args.eta}_d{args.depth}"
...
# -----------------------------------------------------------------------------
# Optimizer setup
def setup_cached_softmax_optimizer(model, eta, svd_freq):
    ...
    Factory = DistCachedSoftmaxMuonAdamW if is_ddp else CachedSoftmaxMuonAdamW
    optimizer = Factory(param_groups, eta=eta, svd_freq=svd_freq)
    for group in optimizer.param_groups:
        group["initial_lr"] = group["lr"]
    return optimizer

optimizer = setup_cached_softmax_optimizer(orig_model, eta=args.eta, svd_freq=args.svd_freq)
print0(f"Cached-SVD SoftmaxMuon optimizer: eta={args.eta}, svd_freq={args.svd_freq}")
...
get_report().log(section=f"CachedSVD SoftmaxMuon (eta={args.eta}, svd_freq={args.svd_freq}) training", data=[
    user_config,
