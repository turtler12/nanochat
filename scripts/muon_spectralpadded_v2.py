...
def muon_step_fused(
    ...
    pad_ratio_t: Tensor,            # () - 0-D CPU tensor, how much to blend in padded polar update (e.g., 0.1)
    power_iters: int,               # 1-2 - power iterations to estimate sigma_max
    power_batch: int,               # 4-8 - number of probe vectors per matrix for sigma_max estimate
    ...
    # Polar express: compute an (approx) polar factor P ≈ U V^T (scale-invariant)
    X = g.bfloat16()
    ...
    # Spectral padding: start from the polar factor (standard Muon), then blend in
    # raw gradient magnitude to lift tail singular values.
    #
    # Standard Muon sets g = P (the polar factor UV^T), discarding all magnitude info.
    # Spectral padding re-injects magnitude: for each singular value σ_i of g_raw,
    # the polar factor has σ_i = 1. We blend toward the raw gradient (which has the
    # original σ_i) to create a floor effect on small singular values:
    #
    #   update = (1-γ) * σ_max * P + γ * g_raw
    #
    # At γ=0: update = σ_max * P, which after variance reduction = standard Muon.
    # At γ>0: tail SVs get lifted from ~0 toward γ*σ_i(g_raw), while leading SVs
    #         stay dominated by (1-γ)*σ_max.
    pad_ratio = pad_ratio_t.to(g.dtype)

    # Estimate σ_max(g_raw) via batched power iteration (cheap: a few GEMMs).
    b = power_batch
    ...
    for _ in range(power_iters):
        ...
        Omega = Omega / (Omega.norm(dim=-2, keepdim=True).clamp_min(1e-6))
    Y = g @ Omega
    sigma_hat = (Y.norm(dim=-2, keepdim=True).amax(dim=-1, keepdim=True)).clamp_min(1e-8)  # (B,1,1)

    # Blend: polar factor scaled by σ_max (base) + raw gradient (spectral detail)
    g = (P.to(g.dtype) * sigma_hat) * (1 - pad_ratio) + g * pad_ratio
    # Variance reduction
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    Arguments:
        param_groups: List of dicts, each containing:
            ...
            - For Muon groups: 'lr', 'momentum', 'ns_steps', 'beta2', 'weight_decay',
              optionally 'pad_ratio' (e.g. 0.1), 'power_iters' (1-2), 'power_batch' (4-8)
    """
    def __init__(self, param_groups: list[dict]):
        ...
        self._muon_pad_ratio_t = torch.tensor(0.0, dtype=torch.float32, device="cpu")
    ...
    def _step_muon(self, group: dict) -> None:
        ...
        muon_step_fused(
            ...
            int(group.get("power_iters", 1)),
            int(group.get("power_batch", 4)),
            group["ns_steps"],
            red_dim,
        )
    ...
    def _compute_muon(self, group: dict, info: dict, gather_list: list, rank: int) -> None:
        ...
        if num_owned > 0:
            ...
            muon_step_fused(
                ...
                self._muon_pad_ratio_t, int(group.get("power_iters", 1)), int(group.get("power_batch", 4)),
