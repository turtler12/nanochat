"""
TaylorMuon optimizer: first-order Taylor approximation of SoftmaxMuon.

Replaces SVD with Newton-Schulz (Polar Express) + a cheap linear correction:
    Q = newton_schulz(M)                    # orthogonal factor (Muon's step)
    M_scaled = M / (||M||_F / sqrt(r))      # normalize M to same Frobenius norm as Q
    update = Q + eta * M_scaled             # first-order Taylor correction
    update = update * (sqrt(r) / ||update||_F)  # renormalize to Muon scale

This is the first-order Taylor expansion of U·diag(softmax(η·S))·Vᵀ around η=0:
- At η=0: softmax gives uniform weights → U·I·Vᵀ = Q (Muon)
- At η>0: first-order correction adds η·U·diag(S - mean(S))·Vᵀ ∝ η·M

Zero SVD calls, fully torch.compile-able, one extra matrix addition beyond Muon.
"""
...
def taylor_muon_step_fused(
    ...
    """
    Fused TaylorMuon step: momentum → polar_express → taylor_correction → variance_reduction → update.
    ...
    # Taylor correction: update = Q + eta * M_scaled
...
class TaylorMuonAdamW(torch.optim.Optimizer):
    """Single-GPU TaylorMuon + AdamW."""
...
class DistTaylorMuonAdamW(torch.optim.Optimizer):
    """Distributed TaylorMuon + AdamW. Same communication pattern as DistMuonAdamW."""
    def __init__(self, param_groups: list[dict], eta: float = 0.5):
        super().__init__(param_groups, defaults={})
        self.eta = eta
        self._adamw_step_t = torch.tensor(0.0, dtype=torch.float32, device="cpu")
